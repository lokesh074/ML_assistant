...
def write_output(agent, output):
    with open("agents_output.md", "a", encoding="utf-8") as f:
        ...
        # Agent title
        f.write(f"## 🧠 Output of Agent: `{agent}`\n\n")

        # Main content block
        if not isinstance(output, str):
            output = str(output)  # Ensure it's string for writing

        # Write output inside a markdown code block for formatting
...
def main():
    ...
    Problem_Definition_Agent = Agent(
        ...
        goal="""clarify the machine learning problem the user wants to solve, 
            identifying the type of problem (e.g., classification, regression,forecasting) and any specific requirements.""",
    ...
    Data_Assessment_Agent = Agent(
        ...
        backstory="""You specialize in data evaluation and preprocessing. 
            ...
            including suggestions for data cleaning and augmentation and how to do data preprocessing and feature engineering""",
        verbose=True,
    ...
    if user_question:
        ...
        result = crew.kickoff()
        write_output("Problem_Definition_Agent",task_define_problem.output.raw_output)
        write_output("Data_Assessment_Agent",task_assess_data.output.raw_output)
        write_output("Model_Recommendation_Agent",task_recommend_model.output.raw_output)
        write_output("Starter_Code_Generator_Agent",task_generate_code.output.raw_output)
...
# Develop a predictive model that can determine the species of an iris flower
...
# i have employee attendance records in which i have to forecast the employee leave count like in next month how many employees will be on leave
